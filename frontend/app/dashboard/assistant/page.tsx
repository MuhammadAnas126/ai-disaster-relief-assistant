...
// One-click prompts, one per core assistant role
...
export default function AssistantPage() {
  ...
  // Ref attached to an anchor at the bottom of the message container
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Smoothly scroll to the bottom whenever the messages array updates or typing indicator toggles
  useEffect(() => {
  ...
  useEffect(() => {
    if (!showSent) return;
    const timer = setTimeout(() => setShowSent(false), 2500);
        ...
        <div className="flex-1 space-y-2 overflow-y-auto pr-1">
          {messages.length === 0 ? (
            <div className="space-y-3">
              <EmptyState
                message={t("assistant.noMessages")}
                hint={t("assistant.noMessagesHint")}
              />
        ...
        <form onSubmit={handleBroadcast} className="space-y-4">
          ...
          {showSent && (
            <p className="text-center text-xs text-success">
              {t("assistant.broadcastSent")}
            </p>
          )}
          {draftLoaded && !showSent && (
            <p className="text-center text-xs text-secondary">
              {t("assistant.draftLoaded")}
            </p>
          )}
        </form>
      </Card>
