...
export default function RescuePage() {
  ...
  async function requestGuidance() {
    ...
    try {
      const satellite = await analyzeSafeSpots(
        await fetchLiveSatelliteImage(position),
      );
      ...
      setSatelliteError(
        err instanceof Error ? err.message : "Live satellite analysis failed",
      );
    } finally {
  ...
  function callForHelp() {
    callAuthority.mutate(
      {
        sessionId: guidance.data?.sessionId,
        lat: position[0],
        lng: position[1],
      ...
      {
        onSuccess: (result) => {
          if (result.authorityPhone)
            window.location.href = `tel:${result.authorityPhone}`;
        },
      },
    );
  }
  ...
  const backHref = pathname.startsWith("/dashboard")
    ? "/dashboard/overview"
    : "/";
    ...
    <div className="mx-auto w-full max-w-[1440px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Link
            href={backHref}
            className="mb-4 inline-flex items-center gap-1.5 text-xs font-medium text-text-muted hover:text-text"
          >
            <ArrowLeft size={14} /> {t("common.back")}
          </Link>
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-accent">
            <ShieldAlert size={15} /> {t("common.appTitle")}
          </div>
        ...
        >
          ...
          {locationState === "shared"
            ? t("rescue.locationShared")
            : t("rescue.shareLocation")}
        </Button>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)]">
        <Card className="h-fit">
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {[
              [t("rescue.quickTrapped"), "I am trapped"],
              [t("rescue.quickFlood"), "Flood water is rising around me"],
              [
                t("rescue.quickEarthquake"),
                "An earthquake damaged the building",
              ],
              [t("rescue.quickFire"), "There is a fire nearby"],
            ].map(([label, value]) => (
              <button
                key={label}
                type="button"
                onClick={() => setSituation(value)}
                className="rounded-xl border border-border px-3 py-2 text-left text-xs font-medium text-text-muted transition-colors hover:border-secondary hover:text-text"
              >
                {label}
              </button>
            ))}
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              <Label htmlFor="rescue-trapped">
                {t("registerCase.trapped")}
              </Label>
              <Select
                id="rescue-trapped"
                value={trapped}
                onChange={(event) =>
                  setTrapped(event.target.value as TrappedStatus)
                }
              >
                <option value="yes">{t("registerCase.trappedYes")}</option>
                <option value="partial">
                  {t("registerCase.trappedPartial")}
                </option>
                <option value="no">{t("registerCase.trappedNo")}</option>
              </Select>
            </div>
            <div>
              <Label htmlFor="rescue-disaster">{t("common.disaster")}</Label>
              <Select
                id="rescue-disaster"
                value={disasterType}
                onChange={(event) => setDisasterType(event.target.value)}
              >
                <option value="">{t("disaster.unknown")}</option>
                <option value="flood">{t("disaster.flood")}</option>
                <option value="earthquake">{t("disaster.earthquake")}</option>
                <option value="fire">{t("disaster.fire")}</option>
                <option value="building_collapse">
                  {t("disaster.building_collapse")}
                </option>
                <option value="other">{t("disaster.other")}</option>
              </Select>
            </div>
          </div>
          <Button
            ...
            disabled={
              !situation.trim() || guidance.isPending || satellitePending
            }
          >
            <Navigation size={17} />{" "}
            {satellitePending
              ? "Analyzing live satellite imagery..."
              : guidance.isPending
                ? t("rescue.generating")
                : t("rescue.getGuidance")}
          </Button>
          <Button
            type="button"
            variant="secondary"
            className="mt-3 w-full border-accent/40 text-accent hover:border-accent"
            onClick={callForHelp}
            disabled={callAuthority.isPending}
          >
            <Phone size={17} />{" "}
            {callAuthority.isPending
              ? t("rescue.callingAuthority")
              : t("rescue.callAuthority")}
          </Button>
          {callAuthority.data && (
            <p className="mt-3 text-center text-xs text-success">
              {t("rescue.notificationSent")} ·{" "}
              {callAuthority.data.authorityPhone}
            </p>
          )}
          {satelliteError && (
            <p className="mt-3 rounded-lg border border-secondary/30 bg-secondary/10 px-3 py-2 text-xs text-secondary">
              {satelliteError}
            </p>
          )}
        </Card>
        ...
        <div className="space-y-4">
          <Card className="overflow-hidden p-0">
            <RescueGuidanceMapClient
              position={position}
              safePoint={activeGuidance?.safePoint ?? null}
            />
            {!activeGuidance && (
              <div className="flex items-center gap-2 border-t border-border px-5 py-3 text-xs text-text-muted">
                <MapPin size={14} /> {t("rescue.noLocation")}
              </div>
            )}
          </Card>
          {guidance.isError && (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Card className="p-4">
                  ...
                  <div className="mt-1 font-semibold text-text">
                    {activeGuidance.safePoint?.label ?? t("rescue.noLocation")}
                  </div>
                </Card>
                <Card className="p-4">
                  <div className="text-xs text-text-muted">
                    {t("rescue.estimatedTime")}
                  </div>
                  <div className="mt-1 font-semibold text-text">
                    {activeGuidance.estimatedTimeMinutes
                      ? `${activeGuidance.estimatedTimeMinutes} ${t("rescue.minutes")}`
                      : "—"}
                  </div>
                </Card>
              </div>
              {activeGuidance.warnings.length > 0 && (
                <div className="rounded-xl border border-secondary/30 bg-secondary/10 p-4">
                  <div className="mb-2 flex items-center gap-2 text-sm font-semibold text-secondary">
                    <AlertTriangle size={16} /> {t("rescue.warning")}
                  </div>
              ...
              <Card>
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="text-xs text-text-muted">
                    {activeGuidance.steps.length} {t("rescue.stepPrefix")}
                  </span>
                </div>
