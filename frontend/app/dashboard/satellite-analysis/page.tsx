...
export default function SatelliteAnalysisPage() {
  ...
  async function searchPlaces(event: FormEvent) {
    ...
    try {
      ...
      if (nextResults.length === 0) setSearchError('No places found. Try a city, landmark, or address.')
    } catch (error) {
      setResults([])
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-accent">
          <ScanSearch size={15} /> Admin tools
        </div>
        <h1 className="text-2xl font-bold text-text">Satellite image analyzer</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Choose an analysis area</CardTitle>
        </CardHeader>
        <form onSubmit={searchPlaces} className="relative flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="min-w-0 flex-1">
            ...
            <Input
              ...
              placeholder="City, landmark, address, or coordinates"
              autoComplete="off"
            />
          </div>
        ...
        {searchError && <p className="mt-3 text-sm text-secondary">{searchError}</p>}
      </Card>

      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
        <Card className="overflow-hidden p-0">
          <SatelliteAnalysisMapClient position={location.position} label={location.label} />
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-5 py-4">
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold text-text">{location.label}</div>
