...
export const LIVE_STREAM_WS_URL = process.env.NEXT_PUBLIC_WS_URL ?? 'ws://localhost:8000/api/livestream/analyze'
...
export function evidenceMediaUrl(path: string | null | undefined): string | null {
  if (!path) return null
...
async function withFallback<T>(real: () => Promise<T>, fallback: () => Promise<T> | T): Promise<T> {
  try {
    ...
    console.warn('[API] Falling back to mock data:', err instanceof Error ? err.message : err)
    return fallback()
  }
}
...
export const incidentsApi = {
  ...
  create: (body: Partial<Incident> & { isGuestReport?: boolean; evidenceIds?: string[] }) =>
    withFallback<Incident>(
...
export const evidenceApi = {
  ...
  upload: async (input: EvidenceUploadInput): Promise<EvidenceRecord> => {
    ...
    if (input.peopleAffected !== undefined) form.append('peopleAffected', String(input.peopleAffected))
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      throw new Error(body?.message ?? body?.detail ?? `Upload failed with status ${res.status}`)
    }
 ...
 */
export function buildOfflineAssistantMessage(context?: AssistantContext): string {
 ...
 */
export function buildOfflineAdminAssistantMessage(context?: AssistantContext): string {
  const isUrdu = context?.language === 'ur'
